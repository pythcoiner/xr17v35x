...
static void serialxr_stop_tx(struct uart_port *port) {
    ...
    if (up->ier & UART_IER_THRI) {
        ...
        if (lcr & 0x80) {
            printk(KERN_INFO
            "channelnum %d: serialxr stop tx - LCR = 0x%x", up->channelnum, lcr);
            serial_out(up, UART_LCR, lcr &
                                     0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
        }
...
static void serialxr_start_tx(struct uart_port *port) {
    ...
    if (!(up->ier & UART_IER_THRI)) {
        ...
        if (lcr & 0x80) {
            printk(KERN_INFO
            "channelnum %d: serialxr start tx - LCR = 0x%x", up->channelnum, lcr);
            serial_out(up, UART_LCR, lcr &
                                     0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
        }
        serial_out(up, UART_IER, up->ier);
...
static void serialxr_stop_rx(struct uart_port *port) {
    ...
    if (lcr & 0x80) {
        printk(KERN_INFO
        "channelnum %d: serialxr stop rx - LCR = 0x%x", up->channelnum, lcr);
        serial_out(up, UART_LCR,
                   lcr & 0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
    }
...
static void serialxr_enable_ms(struct uart_port *port) {
    ...
    if (lcr & 0x80) {
        printk(KERN_INFO
        "channelnum %d: serialxr enable ms - LCR = 0x%x", up->channelnum, lcr);
        serial_out(up, UART_LCR,
                   lcr & 0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
    }
    serial_out(up, UART_IER, up->ier);
...
receive_chars(struct uart_xr_port *up, unsigned int *status) {
        ...
        if (lsr & UART_LSR_BI) {
            ...
            if (up->multidrop_mode == 1) {
                ...
                for (i = 0; i < datasize_in_fifo; i++) {
                    ...
                    if (lcr & 0x80) {
                        printk(KERN_INFO
                        "channelnum %d: receive chars (multidrop mode) - LCR = 0x%x", up->channelnum, lcr);
                        serial_out(up, UART_LCR, lcr &
                                                 0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
                    }
    ...
    for (i = 0; i < datasize_in_fifo; i++) {
        ...
        if (lcr & 0x80) {
            printk(KERN_INFO
            "channelnum %d: receive chars - LCR = 0x%x", up->channelnum, lcr);
            serial_out(up, UART_LCR, lcr &
                                     0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
        }
        ch[i] = serial_in(up, XR_17v35x_UART_RHR);
    ...
    do {
        ...
        if (((xmit->tail + count) & (UART_XMIT_SIZE - 1)) < xmit->tail) {
            ...
            for (i = 0; i < tmp; i++) {
                ...
                if (lcr & 0x80) {
                    printk(KERN_INFO
                    "channelnum %d: transmit_chars1 - LCR = 0x%x", up->channelnum, lcr);
                    serial_out(up, UART_LCR, lcr &
                                             0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
                }
            ...
            for (i = 0; i < count; i++) {
                ...
                if (lcr & 0x80) {
                    printk(KERN_INFO
                    "channelnum %d: transmit_chars2 - LCR = 0x%x", up->channelnum, lcr);
                    serial_out(up, UART_LCR, lcr &
                                             0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
                }
                serial_out(up, XR_17v35x_UART_THR, ch[i]);
...
//	DEBUG_INTR("serialxr_interrupt(%d)...", irq);
    ...
    do {
        ...
        up = list_entry(l,
        struct uart_xr_port, list);

        lcr = serial_in(up, UART_LCR);  // store value of LCR
        if (lcr & 0x80) {
            printk(KERN_INFO
            "channelnum %d: serialxr interrupt - LCR = 0x%x", up->channelnum, lcr);
...
static void serialxr_set_mctrl(struct uart_port *port, unsigned int mctrl) {
    ...
    efr = efr &
          0xEF;    // clear access to shaded registers so that write to MCR does not change from using DTR to RTS for RS-485 control
#if   USE_DTR_RS485
    mcr |= 0x04;
...
static int serialxr_startup(struct uart_port *port) {
...
#if ENABLE_RS485
    serial_out(up, XR_17V35X_EXTENDED_FCTR,
               fctr_reg | XR_17V35X_FCTR_TRGD | XR_17V35X_FCTR_RTS_8DELAY | XR_17V35x_FCTR_RS485);
#if USE_DTR_RS485
    ...
    */
    ...
    if (lcr & 0x80) {
        printk(KERN_INFO
        "channelnum %d: serialxr startup - LCR = 0x%x", up->channelnum, lcr);
        serial_out(up, UART_LCR,
                   lcr & 0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
    }
    serial_out(up, UART_IER, up->ier);
...
static void serialxr_shutdown(struct uart_port *port) {
    ...
    if (lcr & 0x80) {
        printk(KERN_INFO
        "channelnum %d: serialxr_shutdown1 - LCR = 0x%x", up->channelnum, lcr);
        serial_out(up, UART_LCR,
                   lcr & 0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
    }
    serial_out(up, UART_IER, 0);
    ...
    if (lcr & 0x80) {
        printk(KERN_INFO
        "channelnum %d: serialxr_shutdown2 - LCR = 0x%x", up->channelnum, lcr);
        serial_out(up, UART_LCR,
                   lcr & 0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
    }
    serial_out(up, UART_FCR, 0);
...
serialxr_set_special_baudrate(struct uart_port *port, unsigned int special_baudrate) {
    ...
    printk(KERN_INFO
    "Enter in serialxr_set non-standard baudrate:%d channelnum:%d\n", special_baudrate, up->channelnum);
    ...
    if (baud < 12500000 / 16) {//using the 16x mode
        val_4xmode &= ~(1 << port_index);
        val_8xmode &= ~(1 << port_index);
        quot_coeff = 16;
        printk(KERN_INFO
        "Using the 16x Mode\n");
    } else if ((baud >= 12500000 / 16) && (baud < 12500000 / 4)) {//using the 8x mode
        val_4xmode &= ~(1 << port_index);
        val_8xmode |= (1 << port_index);
        quot_coeff = 8;
        printk(KERN_INFO
        "Using the 8x Mode\n");
    } else {//using the 4x mode
        val_4xmode |= (1 << port_index);
        val_8xmode &= ~(1 << port_index);
        quot_coeff = 4;
        printk(KERN_INFO
...
serialxr_set_termios(struct uart_port *port, struct ktermios *termios,
    ...
    if (baud < 120) {
        ...
        serial_out(up, UART_MCR,
                   mcr | 0x80);//set the prescaler (MCR bit-7 = 1, requires EFR bit-4 = 1) to divide the clock by 4.
        ...
        mcr = serial_in(up, UART_MCR);
        serial_out(up, UART_MCR,
                   mcr & 0x7f);//clr the prescaler (MCR bit-7 = 1, requires EFR bit-4 = 1) to divide the clock by 1.
        //Restore the EFR Value
        serial_out(up, XR_17V35X_EXTENDED_EFR, efr);
        low_baudrate_mode = 0;
    }
    ...
    if (lcr & 0x80) {
        printk(KERN_INFO
        "channelnum %d: serialxr_set_termios1 - LCR = 0x%x", up->channelnum, lcr);
        serial_out(up, UART_LCR,
                   lcr & 0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
    }
    serial_out(up, UART_IER, up->ier);
    ...
    */
    ...
    if ((termios->c_iflag) & IXANY) {
        serial_out(up, XR_17V35X_EXTENDED_EFR,
                   ((termios->c_iflag) & IXOFF) || ((termios->c_iflag) & IXON) ? ((reg_read) | 0x1A) : ((reg_read) |
                                                                                                        0x10));
    ...
    if (lcr & 0x80) {
        printk(KERN_INFO
        "channelnum %d: serialxr_set_termios2 - LCR = 0x%x", up->channelnum, lcr);
        serial_out(up, UART_LCR,
                   lcr & 0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
    }
...
serialxr_pm(struct uart_port *port, unsigned int state,
    ...
    if (state) {
        ...
        if (lcr & 0x80) {
            printk(KERN_INFO
            "channelnum %d: serialxr_pm sleep - LCR = 0x%x", up->channelnum, lcr);
            serial_out(up, UART_LCR, lcr &
                                     0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
        }
        ...
        if (lcr & 0x80) {
            printk(KERN_INFO
            "channelnum %d: serialxr_pm wake - LCR = 0x%x", up->channelnum, lcr);
            serial_out(up, UART_LCR, lcr &
                                     0x7f);    // Set LCR bit-7=0 when accessing RHR/THR/IER/ISR to avoid incorrect register access
        }
        serial_out(up, UART_IER, 0);
...
init_one_xrpciserialcard(struct pci_dev *dev, const struct pci_device_id *ent) {
    ...
    for (i = 0; i < nr_ports; i++) {
        ...
        printk(KERN_WARNING
        "init_one_xrpciserialcard line:%d\n", serial_port.line);
...
void pciserial_remove_ports(struct serial_private *priv) {
    ...
    for (i = 0; i < priv->nr; i++) {
        printk(KERN_WARNING
        "pciserial_remove_ports dev:%p port_num:%d\n", priv->dev, priv->uart_index[i]);
...
remove_one_xrpciserialcard(struct pci_dev *dev) {
...
serialxr_init(void) {
    ...
    printk(KERN_INFO
    "Exar PCIe (XR17V35x) serial driver Revision: 2.6\n");
